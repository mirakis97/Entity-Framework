    ...
    public abstract class DbContext
    {
            ...
            using(new ConnectionManager(connection))
            {
                using(var transaction = this.connection.StartTransaction())
                {
                    foreach (IEnumerable dbSet in dbSets)
                    {
                        ...
                        var peresistMethod = typeof(DbContext)
                            .GetMethod("Persist", BindingFlags.Instance | BindingFlags.NonPublic)
                            .MakeGenericMethod(dbSetType);

                        try
                        {
                            peresistMethod.Invoke(this, new object[] { dbSet });
                        }
        ...
        private void InitializeDbSets()
        {
            foreach (var dbSet in this.dbSetProperties)
            {
                ...
                var dtSetProperty = dbSet.Value;

                var populateDbSetGeneric = typeof(DbContext)
                    .GetMethod("PopulateDbSet", BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(dbSetType);

                populateDbSetGeneric.Invoke(this, new object[] { dtSetProperty });
            }
        }
        ...
        private void MapCollection<TDbSet,TCollection>(DbSet<TDbSet> dbSets, PropertyInfo collcetionProperty)
            where TDbSet : class,new() where TCollection : class, new()
        {
            ...
            foreach (var entity in dbSets)
            {
                ...
                ReflectionHelper.ReplaceBackingField(entity, collcetionProperty.Name, navigationEntities);
            }
        }
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet)
                        .First(currentNavinationProperty => navigationPrimaryKey.GetValue(currentNavinationProperty).Equals(foreignKeyValue));
